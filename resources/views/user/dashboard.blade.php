...
@section('title')
    My Dashboard | E-Commerce Mini App
@endsection
...
@section('main-content')
    <div class="max-w-2xl w-full mx-auto px-4 py-10 space-y-5">
        ...
        <div class="text-center">
            ...
            <p class="mt-1 text-sm text-gray-600">Here's what's happening with your orders.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="p-6 bg-white border border-gray-200 rounded-xl shadow">
                ...
                <p class="mt-2 text-2xl font-bold text-gray-900">{{ $orderStats['total'] }}</p>
            </div>
            <div class="p-6 bg-white border border-gray-200 rounded-xl shadow">
                <h3 class="text-sm font-medium text-gray-500">Completed Orders</h3>
                <p class="mt-2 text-2xl font-bold text-green-600">{{ $orderStats['completed'] }}</p>
            </div>
            <div class="p-6 bg-white border border-gray-200 rounded-xl shadow">
                <h3 class="text-sm font-medium text-gray-500">Pending Orders</h3>
                <p class="mt-2 text-2xl font-bold text-yellow-600">{{ $orderStats['pending'] }}</p>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse ($recentOrders as $order)
                            <tr class="text-sm text-gray-700 hover:bg-gray-50">
                                ...
                                <td class="py-3 px-4 border-b">{{ $order->created_at->format('M d, Y') }}</td>
